...
def check_smc_setup(df: pd.DataFrame, htf_trend: str, df_4h: pd.DataFrame = None, htf_timeframes: list = None) -> dict:
    ...
    if ltf_trend == "up":
        ...
        for sl_idx, sl_row in recent_swing_lows.tail(3)[::-1].iterrows():
            last_swing_low_val = sl_row['low']
            ...
            # Nới lỏng: Bỏ RSI (vì RSI thường trễ so với râu nến), Râu dưới >= Body (thay vì 1.5 - 2.0)
            sweep_mask = (post_swing_df['low'] < last_swing_low_val) & (post_swing_df['close'] > last_swing_low_val) & (lower_wick >= body * 1.0)
            ...
            # Nới lỏng: Volume chỉ cần lớn hơn trung bình 20 nến (x1.0) thay vì 1.2-1.5 lần
            ...
            if last_candle['low'] <= fvg['top'] and last_candle['close'] >= fvg['bottom']:
                ...
                reason = (
                    f"{htf_info}{htf_poi_info}. Có Liquidity Sweep (RSI: {sweep_candle['rsi']:.0f}) tại đáy {last_swing_low_val:.2f}"
                    f"{sweep_date_str}. Phá vỡ cấu trúc (MSS) với Vol lớn tại {target_high_val:.2f}"
    ...
    # === KIỂM TRA SHORT SETUP ===
        ...
        for sh_idx, sh_row in recent_swing_highs.tail(3)[::-1].iterrows():
            last_swing_high_val = sh_row['high']
            ...
            # Tìm nến Sweep bằng mảng logic (Vectorized)
            # Nới lỏng: Bỏ RSI, Râu trên >= Body 1.0 lần
            sweep_mask = (post_swing_df['high'] > last_swing_high_val) & (post_swing_df['close'] < last_swing_high_val) & (upper_wick >= body * 1.0)
            valid_sweeps = post_swing_df[sweep_mask]
            ...
            if last_candle['high'] >= fvg['bottom'] and last_candle['close'] <= fvg['top']:
                ...
                reason = (
                    f"{htf_info}{htf_poi_info}. Có Liquidity Sweep (RSI: {sweep_candle['rsi']:.0f}) tại đỉnh {last_swing_high_val:.2f}"
                    f"{sweep_date_str}. Phá vỡ cấu trúc (MSS) với Vol lớn tại {target_low_val:.2f}"
